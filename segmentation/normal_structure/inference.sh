...
for nifti in "${prepared_inputs[@]}"; do
  ...
  for task in "${TASKS[@]}"; do
    ...
    TASK_END=$(date +%s)
    log_time "TSv2-${task}" $((TASK_END - TASK_START))
  done
done
TSV2_END=$(date +%s)
log_time "TSv2-total" $((TSV2_END - TSV2_START))
...
PIPELINE_END=$(date +%s)
log_time "Normal-Structure-Total" $((PIPELINE_END - PIPELINE_START))
echo "[TIMING] ========== 정상 구조물 분할 완료 =========="
