...
class STUNet_DualEncoder(nn.Module):
    def __init__(self, input_channels, num_classes, depth=[1,1,1,1,1,1], dims=[32, 64, 128, 256, 512, 512],
        ...
        # Assume CT is first, PET is second channel/group. If PET is missing, use a zero-filled PET input.
        if input_channels <= 1:
            self.split_sizes = [1, 1]
            print("Input channels = 1. Using CT only and zero-filling PET branch.")
    ...
    def forward(self, x):
        # ==================== FORWARD - ENCODER ====================
        # Split input into CT and PET paths (PET may be missing)
        x_ct = x[:, 0:self.split_sizes[0]]
        if x.shape[1] >= self.split_sizes[0] + self.split_sizes[1]:
            x_pet = x[:, self.split_sizes[0]:self.split_sizes[0] + self.split_sizes[1]]
        else:
            x_pet = torch.zeros(
                (x_ct.shape[0], self.split_sizes[1], *x_ct.shape[2:]),
                device=x_ct.device,
                dtype=x_ct.dtype,
            )
        skips = []
...
class STUNet_DualEncoder_fuse(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.shape[1] >= self.split_sizes[0] + self.split_sizes[1]:
            ...
            x_pet_input = torch.zeros(
                (x_ct_input.shape[0], self.split_sizes[1], *x_ct_input.shape[2:]),
                device=x_ct_input.device,
                dtype=x_ct_input.dtype,
            )
