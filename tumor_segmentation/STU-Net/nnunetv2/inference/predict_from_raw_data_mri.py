...
def run_inference_for_part(file_chunk, part_id, model_folder, temp_base_folder, target_filename):
    """
    一个独立的工作函数，为给定的文件块在指定的GPU上运行推理。
    """
    print(f"--- [Part {part_id}] Starting process for {len(file_chunk)} files on GPU. ---")

    # 1. 创建该任务独立的临时文件夹
    temp_input_folder = join(temp_base_folder, f'temp_input_part_{part_id}')
    temp_output_folder = join(temp_base_folder, f'temp_output_part_{part_id}')
    maybe_mkdir_p(temp_input_folder)
    maybe_mkdir_p(temp_output_folder)

    # 2. 为文件块创建符号链接
    print(f"[{part_id}] Creating symbolic links in {temp_input_folder}...")
    filename_mapping = {}
    for i, original_file_path in enumerate(file_chunk):
        # 确保原始文件存在
        if not exists(original_file_path):
            print(f"[{part_id}] Warning: Source file not found, skipping: {original_file_path}")
            continue

        new_filename_stem = f"case_{i:04d}"
        new_filename_with_modality = f"{new_filename_stem}_0000.nii.gz"
        symlink_path = join(temp_input_folder, new_filename_with_modality)

        filename_mapping[new_filename_stem] = original_file_path

        if os.path.lexists(symlink_path):
            os.remove(symlink_path)
        os.symlink(abspath(original_file_path), symlink_path)

    if not filename_mapping:
        print(f"[{part_id}] No valid files to process after checking existence. Exiting.")
    ...
    try:
        ...
        predictor.initialize_from_trained_model_folder(
            model_folder,
            use_folds=(0, 1, 2, 3, 4),  # 使用你指定的 fold 0
            checkpoint_name='checkpoint_final.pth',
        )

        print(f"[{part_id}] Starting prediction...")
        predictor.predict_from_files(
            list_of_lists_or_source_folder=temp_input_folder,
            output_folder_or_list_of_truncated_output_files=temp_output_folder,
        ...
        print(f"[{part_id}] !!! An error occurred during nnU-Net prediction: {e}")
        # 清理并退出
        shutil.rmtree(temp_input_folder)
        shutil.rmtree(temp_output_folder)
        exit(1)
    ...
    # 4. 移动并重命名结果
    print(f"[{part_id}] Moving and renaming results...")
    predicted_files = sorted(glob(join(temp_output_folder, '*.nii.gz')))
    for predicted_file_path in predicted_files:
        predicted_stem = basename(predicted_file_path).replace('.nii.gz', '')

        if predicted_stem in filename_mapping:
            original_input_path = filename_mapping[predicted_stem]
            # 目标文件夹是 '.../sinwas/' 的上一级目录，即日期文件夹
            target_folder = dirname(dirname(original_input_path))
            final_output_path = join(target_folder, target_filename)

            print(f"[{part_id}]   -> Moving result to {final_output_path}")
            shutil.move(predicted_file_path, final_output_path)

    # 5. 清理
    print(f"[{part_id}] Cleaning up temporary folders.")
    shutil.rmtree(temp_input_folder)
    shutil.rmtree(temp_output_folder)
    print(f"--- [Part {part_id}] Process completed successfully. ---")
...
if __name__ == '__main__':
    ...
    # 2. 定义关键路径和参数
    PART_ID_TO_RUN = 0
    JSON_FILE_PATH = '/projects/pet_ct_challenge/tianyu/models/nnUNet_trained_models/Dataset261_nnunet_tumor/nnUNetTrainer__nnUNetPlans__3d_fullres/test.json'
    NUM_PARTS = 4  # 总共分成4份

    TEMP_BASE_FOLDER = '/projects/nki-breast-mri/dicom2nifti/NKI/MRI/temp'
    # 注意：这里的模型文件夹是你代码片段中的 Dataset104
    MODEL_FOLDER = join(nnUNet_results, 'Dataset261_nnunet_tumor/nnUNetTrainer__nnUNetPlans__3d_fullres')

    # 定义GPU映射关系，假设你有4个GPU (0, 1, 2, 3)
    GPU_MAP = {0: 0, 1: 1, 2: 2, 3: 3}
    MY_GPU_ID = GPU_MAP[PART_ID_TO_RUN]

    # 定义目标输出文件名
    TARGET_FILENAME = 'UNET_predicted_segmentation_DCE-binary_mask_tumor.nii.gz'

    # 3. 读取JSON文件获取完整列表
    try:
        with open(JSON_FILE_PATH, 'r') as f:
            all_files_to_predict = json.load(f)
        print(f"Successfully loaded {len(all_files_to_predict)} file paths from {JSON_FILE_PATH}.")
    except Exception as e:
        print(f"Error: Could not read or parse JSON file at {JSON_FILE_PATH}. Error: {e}")
        exit(1)

    # 4. 过滤已完成的文件 (断点续传功能)
    # print("Filtering out cases where prediction already exists...")
    # files_needing_prediction = []
    # for file_path in all_files_to_predict:
    #     output_dir = dirname(dirname(file_path))  # 日期文件夹
    #     expected_output_file = join(output_dir, TARGET_FILENAME)
    #     if not exists(expected_output_file):
    #         files_needing_prediction.append(file_path)
    #
    # print(f"Found {len(files_needing_prediction)} files that need prediction.")
    #
    # if not files_needing_prediction:
    #     print("All cases have already been processed. Exiting.")
    #     exit()
    if not all_files_to_predict:
        print("The JSON file is empty. Nothing to do. Exiting.")
        exit()

        # 5. 将 *完整的* 文件列表分配给各个部分
    files_per_part = [[] for _ in range(NUM_PARTS)]
    # 直接遍历 all_files_to_predict
    for i, f in enumerate(all_files_to_predict):
        files_per_part[i % NUM_PARTS].append(f)

    my_file_chunk = files_per_part[PART_ID_TO_RUN]


    if not my_file_chunk:
        print(f"Part {PART_ID_TO_RUN} has no files to process in this run. Exiting.")
        exit()

    # 6. 调用工作函数
    run_inference_for_part(
        file_chunk=my_file_chunk,
        part_id=PART_ID_TO_RUN,
        model_folder=MODEL_FOLDER,
        temp_base_folder=TEMP_BASE_FOLDER,
