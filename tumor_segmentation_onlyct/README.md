...
- `postprocess_merge_labels.py`: 추론 결과의 라벨(1/2)을 1로 통합
- `nnUNet/`: nnU-Net v2 소스(Editable install)
- `tumor_segmentation_onlyct_model/`: 사전 학습된 모델 가중치

## 설치
```bash
bash ./tumor_segmentation_onlyct/requirements.sh
```
...
필요하면 직접 생성하세요.
```bash
mkdir -p ./tumor_segmentation_onlyct/input_folder
mkdir -p ./tumor_segmentation_onlyct/output_folder
```
...
- Checkpoint: `tumor_segmentation_onlyct_model/Dataset901_HECKTOR25/nnUNetTrainer__nnUNetResEncUNetLPlans__3d_fullres/fold_0/checkpoint_final.pth`
...
- 추론 후 `postprocess_merge_labels.py`가 실행되어 라벨 1/2는 단일 라벨 1로 합쳐집니다.
...
## 수정사항 (2026-02-10)
- `ModuleNotFoundError: No module named 'numpy._core'` 대안으로 `numpy` 삭제 후 재설치 추가
